...
public abstract class CarparkingLauncher2 {
	static ArrayList<Agent> cars = new ArrayList<Agent>();
	...
	public static void main(String[] args) throws InterruptedException {
		...
		envAgent.getBeliefs().declareObject( c4 );
		envAgent.getBeliefs().declareObject( c5 );
		envAgent.getBeliefs().declareObject( c6 );
		for(int i = r-1; i >= 0; i--){
			for(int j = 0; j < c; j++) {
...
//		envAgent.startInSeparateThread();
		...
		Agent c4Agent = createAgentCar(c4,false,grid[10][6],envAgent);
		Agent c5Agent = createAgentCar(c5,false,grid[0][3],envAgent);
		Agent c6Agent = createAgentCar(c6,true,grid[11][6],envAgent);
...
//		c3Agent.startInSeparateThread();
		Environment.addAgent (c4Agent);
//		c3Agent.startInSeparateThread();
		Environment.addAgent (c5Agent);
//		c3Agent.startInSeparateThread();
		Environment.addAgent (c6Agent);
//		c3Agent.startInSeparateThread();
		
		
		//cars.add(c0Agent);
		Environment.GUI.InitPanelInfo(cars);
		...
		PddlClause[] c4goals = {Carparking.sayOn(c4, grid[0][6])};
		Message msg4 = addGoalToAgent(c4, c4goals);

		PddlClause[] c5goals = {Carparking.sayOn(c5, grid[7][6])};
		Message msg5 = addGoalToAgent(c5, c5goals);

		PddlClause[] c6goals = {Carparking.sayOn(c6, grid[4][0])};
		Message msg6 = addGoalToAgent(c6, c6goals);
		...
		envAgent.getBeliefs().declare(Carparking.sayMe(c2));
		envAgent.getBeliefs().declare(Carparking.sayMe(c3));
		envAgent.getBeliefs().declare(Carparking.sayMe(c4));
		envAgent.getBeliefs().declare(Carparking.sayMe(c5));
		envAgent.getBeliefs().declare(Carparking.sayMe(c6));
		...
		envAgent.getBeliefs().declare( Carparking.sayCar(c4) );
		envAgent.getBeliefs().declare( Carparking.sayOn(c4, grid[10][6]) );

		envAgent.getBeliefs().declare( Carparking.sayCar(c5) );
		envAgent.getBeliefs().declare( Carparking.sayOn(c5, grid[0][3]) );
		

		envAgent.getBeliefs().declare( Carparking.saySlowcar(c6) );
		envAgent.getBeliefs().declare( Carparking.sayOn(c6, grid[11][6]) );
		envAgent.getBeliefs().declare( Carparking.sayEmpty(c6));
		/*
		...
		c3Agent.startInSeparateThread();
		//Environment.addAgent (c3Agent);
		c4Agent.startInSeparateThread();
		//Environment.addAgent (c3Agent);
		c5Agent.startInSeparateThread();
		//Environment.addAgent (c3Agent);
		c6Agent.startInSeparateThread();
...
//		Environment.sendMessage ( msg3 );
		...
		InitGUIGoals(0,6,Color.orange);
		InitGUIGoals(7,6,Color.pink);
		InitGUIGoals(4,0,Color.gray);
